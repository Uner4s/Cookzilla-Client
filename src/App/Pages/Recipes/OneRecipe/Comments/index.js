...
@withMessage
@withMutation(gql`mutation createComment($recipeId: String!, $content: String!){
...
export default class Comments extends React.Component {
  ...
  static propTypes = {
    commentRecipes: React.PropTypes.func,
  ...
  async renderComment (variable) {
    try {
      ...
      await this.props.createComment({recipeId: this.props.recipeId, content: variable})
    } catch (error) {
  ...
  render () {
    return (
      <div className={styles.container}>
        ...
        <Button label='Send' onClick={() => this.renderComment(this.state.comment)}/>
        {this.renderComments()}
